...
def criar_tabelas():
    conn = obter_conexao()
    conn.autocommit = True # Facilita atualizações na estrutura
    c = conn.cursor()
    ...
    # Tenta adicionar a coluna local_jogo caso a tabela partidas seja da versão antiga
    try:
...
# Inicializa as tabelas ao rodar o app
criar_tabelas()
...
# Criação das abas de navegação no topo
aba_registro, aba_linha, aba_goleiros = st.tabs([
    "📝 Registrar Partida", 
...
with aba_registro:
    with st.form("registro_partida", clear_on_submit=True):
        ...
        for i in range(5): 
            ...
            if nome:
                jogadores_a.append({"nome": nome, "gols":打gols, "assistencias": asts})

        st.markdown("---")
        
        # --- PREENCHIMENTO DO TIME B ---
        st.subheader("👕 Time B")
...
with aba_linha:
    ...
    if not df_linha.empty:
        # ATUALIZAÇÃO DA FÓRMULA: Média de Participação em Gols = (Gols + Assistências) / Jogos
        df_linha["Média Participação/Jogo"] = (
            (df_linha["Gols"] + df_linha["Assistências"]) / df_linha["Jogos"]
        ).round(2)
        
        # Reorganiza as colunas para o visual ficar excelente
        colunas_linha = ["Jogador", "Jogos", "Títulos", "Gols", "Assistências", "Média Participação/Jogo"]
        df_linha = df_linha[colunas_linha]
        
        # Ordena o ranking pela Média de Participação (do maior para o menor)
...
with aba_goleiros:
    ...
    if not df_goleiros.empty:
        ...
        df_goleiros["% Pên. Defendidos"] = (
            df_goleiros["Pên. Defendidos"] / df_goleiros["Pênaltis Contra"] * 100
        ...
        df_goleiros = df_goleiros[colunas_finais]

        df_goleiros = df_goleiros.sort_values(by="Média Sofridos/Jogo", ascending=True)
